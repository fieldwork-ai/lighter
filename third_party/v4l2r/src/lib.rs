...
//!
// Vendored: a newer rustc's lint on elided lifetimes, not this crate's concern.
#![allow(mismatched_lifetime_syntaxes)]

#[doc(hidden)]
pub mod bindings;
pub mod controls;
// The device, decoder and encoder modules drive a V4L2 device through the
// kernel (epoll, eventfd) and are Linux-only; lighter uses the types and
// the ioctl encoders on macOS, where it is the device.
#[cfg(target_os = "linux")]
pub mod decoder;
#[cfg(target_os = "linux")]
pub mod device;
