...
/// The base and the range for a guest of `total` bytes that boots on `base`.
///
/// The caller chooses the base (the CLI's native mode: two gigabytes). The kernel's
...
///
/// `LIGHTER_VIRTIO_MEM=0` gives the guest everything at boot, for the A/B;
/// `LIGHTER_VIRTIO_MEM=<MiB>` sets the base by hand.
pub fn split(total: u64, base: u64) -> (u64, u64) {
...
mod tests {
    ...
    #[test]
    fn the_base_is_whole_blocks_and_the_rest_is_the_range() {
        // Not through the environment: a parallel test may be reading it.
        if std::env::var_os("LIGHTER_VIRTIO_MEM").is_some() {
            return;
        }
        assert_eq!(split(46 << 30, 2 << 30), (2 << 30, 44 << 30));
        assert_eq!(split(6 << 30, 2 << 30), (2 << 30, 4 << 30));
        assert_eq!(
            split(6 << 30, (2 << 30) + 1),
            ((2 << 30) + BLOCK_SIZE, (4 << 30) - BLOCK_SIZE)
        );
        assert_eq!(split(2 << 30, 2 << 30), (2 << 30, 0));
        assert_eq!(split(1 << 30, 2 << 30), (1 << 30, 0));
    }
