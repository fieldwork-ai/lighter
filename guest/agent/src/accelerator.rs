...
//!
//! So the cap is raised for exactly as long as a stream to an accelerator
//! port is open, and put back when the last one closes; the floor stays at
//! rest. Raising the cap alone is enough because the guest kernel grows the
//! window only on a wakeup from outside, a reply from the host or another
//! vCPU's hand-off, never on its own timer (patch 0011): while a model is
//! answering, every message is such a wakeup and the window sits at the
//! cap; with the stream idle, the guest's timers are all that fire and the
//! window falls back to the floor within a few rounds. A resident client
//! that holds its connection for its lifetime, Frigate's detector or a
//! whisper server, therefore costs the Mac nothing between requests: the
//! first shape of this, which held a 2 ms floor as well, spun a full host
//! core for the M1's idle home server. The ports come from the kernel
//! command line, where init also reads them for the CDI specs. The sysfs
//! knob is the patch's module parameter.
//!
...
/// The cap while a stream is open, nanoseconds. On the M1, Qwen2.5-0.5B
/// over `lighter.sh/metal`: 64 tokens a second at the resting 200 µs, 76
/// at 2 ms, 95 to 99 at 5 ms, no more at 20 or 50; a token's wait for the
/// logits is nine milliseconds, so 5 ms keeps a vCPU awake between the
/// messages and lets it sleep through the compute.
...
impl Wide {
    pub fn open(port: u16) -> Option<Wide> {
        ...
        if open.0 == 0 {
            let resting = std::fs::read_to_string(POLL_NS).ok()?;
            std::fs::write(POLL_NS, WIDE_NS).ok()?;
            open.1 = Some(resting);
        }
        open.0 += 1;
        Some(Wide(()))
...
impl Drop for Wide {
    fn drop(&mut self) {
        ...
        open.0 -= 1;
        if open.0 == 0
            && let Some(resting) = open.1.take()
        {
            let _ = std::fs::write(POLL_NS, resting.trim());
        }
    }
}
