...
including the model load, on an M1:

| model | container, `lighter.sh/metal` | container, CPU | native Metal |
|---|---:|---:|---:|
| base.en | 1.11 s | 1.46 s | 0.38 s |
| small.en | 1.58 s | 5.26 s | 1.27 s |

The larger the model, the closer to native: small.en's encoder is 4.1 s on
the container's CPU and a few milliseconds of GPU time, and the remaining
gap to native is the decoder's small graphs crossing the streams once per
token. Through Home Assistant's Wyoming path, 3.5 s of speech comes back as
text in 0.5 s with small.en.
