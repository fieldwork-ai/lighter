...
`LIGHTER_LINK_TRACE=1` on the machine prints, once a second to its stderr (`machine.log` under a `lighter start`, the stage log under the suite), the link's counters — frames in and out, loop iterations, wakes, kqueue events, dropped frames — and one line per stream with its phase, its smoltcp state, how much sits in each ring, and whether the Mac side is being read or written. It is the one probe that works when the control channel is what has stopped, since the control channel rides the link: a stream with bytes in its receive ring and `writing=false` is one the reactor is not draining; one at `await-header` for longer than a moment is a guest that connected and never sent its destination; a `frames_in` that stops moving while the guest is busy is a card nobody is reading. Pair it with `sample <pid> 2` for the host side. The guest reports from its side too: after three consecutive timeouts connecting to the host, the agent writes sysrq's memory summary, blocked tasks and every task's stack to the console, once per boot, since the control channel that would carry a diagnosis rides the link that has stopped.
