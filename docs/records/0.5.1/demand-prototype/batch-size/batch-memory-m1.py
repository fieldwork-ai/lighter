...
p=argparse.ArgumentParser();p.add_argument('--arm',type=int);p.add_argument('--home',type=Path);p.add_argument('--mode',choices=['64k','256k']);a=p.parse_args()
cli=base/('lighter-m1-'+(a.mode or '64k'));guest=root/'guest/out';out=base/'m1-batch-a1'
...
if a.arm is not None:
 ...
 env.update(LIGHTER_HOME=str(a.home),DOCKER_HOST='unix://'+str(a.home/'docker.sock'),LIGHTER_DEMAND_RAM='1',LIGHTER_DEMAND_BASE='1',LIGHTER_BACKGROUND_RAM='1')
 ...
 with (arm/'commands.log').open('w') as log:
  ...
  try:
   run([cli,'start']);run([cli,'stop']);time.sleep(2)
   for rep in range(3):
...
env.update(LIGHTER_HOME=str(home),DOCKER_HOST='unix://'+str(home/'docker.sock'),LIGHTER_DEMAND_RAM='1',LIGHTER_DEMAND_BASE='1',LIGHTER_BACKGROUND_RAM='1')
artifacts={str(f):hashlib.file_digest(f.open('rb'),'sha256').hexdigest() for f in [cli,base/'lighter-m1-256k',base/'source-256k.patch',guest/'Image',guest/'rootfs.ext4',Path(__file__),root/'.logs/050/benchmark-images/arm64.tar']}
order=['64k','256k','64k'];(out/'environment.json').write_text(json.dumps(dict(artifacts=artifacts,source=sp.check_output(['git','rev-parse','HEAD'],cwd=source,text=True).strip(),order=order,profile=json.loads((home/'config.json').read_text()),program=program),indent=2)+'\n')
passed=False
try:
