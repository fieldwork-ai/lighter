...
static void *peer(void *arg) {
 ...
 send(fd,b,284,0); recv(fd,b,1,0);
 send(fd,b,23,0);sched_yield(); shutdown(fd,SHUT_WR);
 ssize_t n; while((n=recv(fd,b,sizeof(b),0))>0){}
 if(n<0){int e=errno;ssize_t peek=recv(fd,b,1,MSG_PEEK|MSG_DONTWAIT);printf("PEER_HUNG errno=%d peek=%zd\n",e,peek);fflush(stdout);peer_failed++;}
 return NULL;
}
int main(int argc,char **argv){
 ...
 for(int i=0;i<runs;i++){
  ...
  ssize_t n=recv(fds[0],buf,1<<20,0);send(fds[0],"x",1,0);
  while((n=recv(fds[0],buf,1<<20,0))>0){}
  if(n<0){int e=errno; ssize_t peek=recv(fds[0],buf,1,MSG_PEEK|MSG_DONTWAIT);printf("HUNG trial=%d errno=%d peek=%zd\n",i,e,peek);fflush(stdout);failed++;}
