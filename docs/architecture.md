...
So the descriptor is treated as a cache of where the file lives, never as the file's identity. A budget proportional to the ceiling bounds how many stay open; a clock sweep parks the cold ones — close the descriptor, remember the path, reopen on next use, and if the path has moved in the meantime reopen by identity through macOS's `/.vol` namespace, with the file's immutable birth time as the check that a recycled inode number is not impersonating it; and the sweep escalates in three regimes — deferring to recency while the budget holds, pacing itself while a hot working set breathes above it, and parking regardless of recency at a red line short of the ceiling, because past that the next event is `EMFILE` inside the guest on a file that is plainly there.
...
So creates, writes, unlinks and clones are acknowledged as soon as their outcome is known and performed in order on one apply queue (`crates/lighter-fs/src/apply.rs`). A pending create is a registry inode with a provisional identity and the attributes it was promised; lookups consult a per-directory overlay of promised and promised-away names before asking the host; sizes come from an overlay while writes are queued. Three promises hold it up, in order of importance: reads never lie (anything whose answer a queued job could change either consults the overlay or settles first — scoped to the inode's own sequence mark, not the whole queue, which under a backlog is the difference between a barrier and a stall); durability is never claimed early (`fsync`, `syncfs` and `DESTROY` drain fully — after them the host answers for everything); and errors are not swallowed (a failed job parks its errno on the inode for the next operation to report; near a full disk acknowledgement stops and service goes back to synchronous). What the guest gives up is a visibility window: a file a container wrote reaches the Mac milliseconds after the write returned rather than before. The other direction — the Mac's changes reaching the guest — is untouched, and the server stays authoritative over every attribute, which is what keeps the writeback class of bugs out. `LIGHTER_FS_ASYNC=0` restores synchronous service.

Two things had to be true for it to pay, and both were measured the hard way. **One thread owns all mutation.** Queueing writes alone made installs slower: the drainer's allocations contended inside APFS against the creates still on the critical path, and the loss exceeded the gain. Only once creates joined the queue did the critical path become acknowledgements and the drainer the sole party APFS hears from. And the queue is a bounded window, because an unbounded one let the guest race the whole install ahead of the disk and every remaining synchronous operation then paid the contention. **The registry must be able to breathe.** A parked directory taxes every name inside it, so directories are residency-privileged in every sweep; and the lookup count must match the kernel's exactly — the server finding its own entry for a name is not a lookup, and counting it as one once left every unlinked file a FORGET short of release, the table climbing past the budget, and a one-second tree walk taking sixteen minutes.
