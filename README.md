...
- **A tick rate per machine:** Creating and tearing down a container's network waits on RCU grace periods, which end on the scheduler tick, and a container's life is a chain of them. The guest asks for the expedited kind where it can, and for the rest lighter ships two kernels that differ only in the tick rate: 1000 a second where the vCPUs are at most half the Mac's cores, which takes a container's life from 112 to 60 ms on an M5 Pro, and 250 where they fill them, because every tick is an exit from the VM per vCPU and on an M1 a thousand of them cost more than they saved.
