...
- **Cooperative resources (experimental):** `lighter config --resources cooperative` stops sizing the machine: every core is a vCPU, memory grows as containers need it and goes back when they stop, up to twice the Mac's RAM with macOS compressing and swapping past it as it would for a native app, and a burst that outruns the growth is slowed rather than OOM-killed. `--cpus` and `--memory` become limits. See the [0.10.0 release notes](docs/release-notes-0.10.0.md).
