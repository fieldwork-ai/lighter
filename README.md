...
- **Mac-native resources (experimental):** `lighter config --resources native` stops sizing the machine: every core is a vCPU, memory grows up to the Mac's RAM less 2 GiB as containers need it and goes back when they stop, and a burst that outruns the growth is slowed rather than OOM-killed. `--cpus` and `--memory` become limits. See the [0.10.0 release notes](docs/release-notes-0.10.0.md).
