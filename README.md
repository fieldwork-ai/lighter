...
|---|---|---|---|---|---|
| `npm ci` | 6.16 s | **4.49 s** (137%) | 7.01 s (88%) | 8.59 s (72%) | 8.61 s (72%) |
| `pnpm install` | 3.77 s | **1.13 s** (334%) | 2.03 s (185%) | 1.14 s (332%) | 2.87 s (131%) |
| `yarn install` | 5.75 s | **4.01 s** (143%) | 5.08 s (113%) | 6.58 s (87%) | 11.14 s (52%) |
| `ripgrep` (file read) | 927 ms | **81 ms** (1144%) | 102 ms (909%) | 121 ms (766%) | 124 ms (748%) |
| `find` (metadata walk) | 357 ms | **89 ms** (401%) | 127 ms (281%) | 176 ms (203%) | 131 ms (273%) |
| `cp -a node_modules` | 13.55 s | **907 ms** (1494%) | 1.11 s (1216%) | 1.88 s (722%) | 2.58 s (526%) |
| `rm -rf node_modules` | 3.65 s | **379 ms** (964%) | 496 ms (737%) | 551 ms (663%) | 428 ms (854%) |

| Workload (host share) | native APFS | lighter | OrbStack | Colima | Docker Desktop |
|---|---|---|---|---|---|
| `npm ci` | 6.16 s | **6.17 s** (100%) | 8.49 s (73%) | 17.79 s (35%) | 17.91 s (34%) |
| `pnpm install` | 3.77 s | **3.88 s** (97%) | 4.72 s (80%) | 25.43 s (15%) | 28.34 s (13%) |
| `yarn install` | 5.75 s | **5.03 s** (114%) | 7.79 s (74%) | 22.16 s (26%) | 22.58 s (25%) |
...
| `cp -a node_modules` | 13.55 s | **3.32 s** (408%) | 8.71 s (156%) | 44.30 s (31%) | 33.55 s (40%) |
| `rm -rf node_modules` | 3.65 s | **2.45 s** (149%) | 2.97 s (123%) | 8.05 s (45%) | 6.56 s (56%) |
...
|---|---|---|---|---|
| Idle, a minute after start | **366 MiB** | 1197 MiB | 11361 MiB | 2111 MiB |
| Peak through an npm install | **2151 MiB** | 5498 MiB | 8700 MiB | 9182 MiB |
| 15 s after it ends | **814 MiB** | 2850 MiB | 8735 MiB | 9187 MiB |
| 60 s after it ends | **814 MiB** | 2114 MiB | 8735 MiB | 9187 MiB |
...
|---|---|---|---|---|---|---|
| TCP, container to the Mac | Gbit/s | 123.5 | 95.8 | **97.2** | 4.5 | 23.2 |
| TCP, the Mac to a container | Gbit/s | 129.2 | **97.5** | 52.9 | 3.9 | 14.3 |
| TCP into a published port | Gbit/s | — | **93.6** | 54.2 | 3.8 | 14.3 |
| TCP out of a published port | Gbit/s | — | **96.2** | 93.1 | 4.4 | 33.4 |
...
| GET on a published port, median | µs | 40 | **56** | 73 | 224 | 119 |
| GET on a published port, p99 | µs | 70 | 138 | **119** | 361 | 245 |
...
| Wakeups per second | 62 | 99 | **50** | 3748 |
...
|---|---|---|---|---|---|
| `npm ci` | 4.49 s | **9.28 s** | 13.16 s | 12.71 s | 14.45 s |
| `pnpm install` | 1.13 s | 2.98 s | 4.44 s | **2.70 s** | 3.89 s |
