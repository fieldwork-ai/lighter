...
Canonical full records: [M5 share](results/lighter.csv), [guest disk](results/lighter-guest.csv), [amd64](results/lighter-amd64.csv); [M1 share](results/machines/m1/lighter.csv), [guest disk](results/machines/m1/lighter-guest.csv), [amd64](results/machines/m1/lighter-amd64.csv). Adjacent `.tree` files record source and artifact stamps. Cache comparisons retain every ordered sample, including rejected profiles.

The M5 observations retain sanitized CPU readings and the initial failed speed-gate sample. CPU rows sum ps-reported percentages over listed processes, including the benchmark; this is not a background-only or total-machine CPU reading. A blank filesystem-daemon value means it was outside the sampled top sixteen processes, not zero CPU.
