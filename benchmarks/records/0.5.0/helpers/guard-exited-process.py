        ...
        def sample(phase):
            ...
            for pid, process in table.items():
                ...
                if a.target in {"orbstack", "docker-desktop"} and kind == "hypervisor" and not legitimate:
                    ...
                    import ctypes
                    try:
                        get_owner = ctypes.CDLL("/usr/lib/libSystem.B.dylib", use_errno=True).responsibility_get_pid_responsible_for_pid
                        get_owner.argtypes = [ctypes.c_int]
                        get_owner.restype = ctypes.c_int
                        owner = get_owner(pid)
                        process["responsibility_probe"] = {"pid": owner, "errno": ctypes.get_errno() if owner < 0 else 0}
