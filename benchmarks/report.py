...
def sources(results):
    """Pinned report inputs, or local CSVs for an uncurated result directory."""
    selection = results / "selection.json"
    if selection.exists():
        manifest = json.loads(selection.read_text())
        if manifest["schema"] != 1:
            raise ValueError(f"unsupported selection schema: {selection}")
        selected = {
            target: {kind: results / path for kind, path in paths.items()}
            for target, paths in manifest["targets"].items()
        }
        for target, paths in selected.items():
            if "csv" not in paths:
                raise ValueError(f"missing CSV selection for {target}: {selection}")
            for path in paths.values():
                if not path.is_file():
                    raise FileNotFoundError(f"selected benchmark input is missing: {path}")
        return selected
    return {p.stem: {"csv": p} for p in results.glob("*.csv")}


def load(target, results=RESULTS):
    source = sources(results).get(target)
    if source is None:
        return {}
    path = source["csv"]
    runs = {}
    with path.open() as handle:
        for row in csv.DictReader(handle):
            if row["ms"] == "timeout":
                ...
                print(
                    f"note: {target} {row['case']} rep {row['rep']} timed out",
                    file=sys.stderr,
                )
                continue
...
def describe_this_machine():
    ...
    try:
        ...
        return (
            out.stdout.strip() or f"{platform.machine()}, macOS {platform.mac_ver()[0]}"
        )
    except Exception:
...
def machines():
    ...
    note = RESULTS / "machine.txt"
    sets = [
        (
            RESULTS,
            note.read_text().strip() if note.exists() else describe_this_machine(),
            True,
        )
    ]
    for entry in sorted((RESULTS / "machines").glob("*/")):
        note = entry / "machine.txt"
        if note.exists() and sources(entry):
...
def main():
    lines = [
        "# Benchmark results",
        "",
        "Generated by `benchmarks/report.py` from the record paths selected in",
        "`benchmarks/results/selection.json` and `benchmarks/results/machines/*/selection.json`.",
    ...
    if network_columns:
        ...
        for case, label, unit, _, divisor in NETWORK_CASES:
            if not any(case in values for _, values in network_columns):
                continue
            cells = [
                network_cell(values.get(case), divisor) for _, values in network_columns
            ]
