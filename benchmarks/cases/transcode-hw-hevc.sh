#!/bin/sh
# The media engine: the x265 case's five seconds of 1080p30 encoded in
...
set -eu
if ffmpeg -hide_banner -encoders 2>/dev/null | grep -q " hevc_videotoolbox "; then enc=hevc_videotoolbox
elif ls /dev/video* >/dev/null 2>&1; then enc=hevc_v4l2m2m
