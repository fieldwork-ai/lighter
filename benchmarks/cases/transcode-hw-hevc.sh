#!/bin/sh
# The media engine: the clip decoded and encoded in hardware, H.264 to HEVC
...
if ffmpeg -hide_banner -encoders 2>/dev/null | grep -q " hevc_videotoolbox "; then
	ffmpeg -hide_banner -loglevel error -hwaccel videotoolbox -i "$in" -an -c:v hevc_videotoolbox -b:v 8M -f null -
