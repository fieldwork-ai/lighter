...
# The rest of the CLI's idle-poll policy (`config::idle_poll_args`).
bench_idle_poll_args() {
	local cores; cores="$(sysctl -n hw.ncpu)"
	if [ "${BENCH_CPUS:-8}" -ge "$cores" ]; then echo "idle.poll_timeouts=0 idle.poll_cost_ns=100000"; fi
}
