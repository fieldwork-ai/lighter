...
def unit(case):
 ...
 return 'ms'
lines=['# 0.4.2 release measurements','','The primary record is the first full M1 suite, selected before the run. Two further consecutive suites measure repeatability and exercise filesystem-daemon recovery. No M5 performance measurements were taken for this release.','','The comparison baseline is the complete 0.4.1 run after the user restarted fseventsd on 2026-09-07, source `5f48cf03` (the released 0.4.1 runtime). The new suites use `cc717e79`; runtime `7d2123d` is unchanged by the final formula checksum commit. Each stage starts a fresh VM with eight vCPUs, 4 GiB RAM and a 128 GiB sparse data disk. The boot case uses the same CLI defaults as the baseline. Each timing case has an untimed warm-up and three ordered repetitions; memory and power rows are single sampling windows.','','The baseline and new suites use the same benchmark cases and the same M1. Six consecutive quiet samples ten seconds apart precede the first new suite. There is no daemon reset between suites. These are cross-session numeric changes, not causal speedup estimates. Workload, cache, thermal and background variation remain possible. The three-run CV is descriptive and is not a universal regression threshold.','','Raw CSVs, source/artifact stamps, monitoring samples and the exact soak protocol are retained in [the release records](results/releases/0.4.2/).']
